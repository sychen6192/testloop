...
撤回時刪掉這批新增的、以及還原的原始碼與資源對應的舊輸出（下一次建置從還原的原始碼重產）。
被中斷（Ctrl-C、crash）時，正在跑的那一批比照失敗批次撤回：它的測試還沒通過任何 gate。
...
「同樣的原因」是最後一輪的 gate 報告拿掉各批自己的類別名稱與所有數字後逐字相同——兩個不同類別的建置
失敗成一模一樣，失敗的是兩者都沒寫的東西（surefire 的 JVM 當掉、相依解析不到、上游模組壞了）。只看建置：
覆蓋率與 review 的報告本來就是各類別的事，拿掉名稱與數字後常常一樣，拿來比會把正常的失敗當成外部問題。
