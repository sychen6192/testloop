...
`testgen` 是一條放在 **Java repo 的 `tools/testgen/`** 底下的 pipeline（不是 global 工具、不是
`~/.config/opencode/`）。它對指定的 Java 類別執行「產生 → 編譯/測試 gate → 覆蓋率 gate →
...
npx tsc --noEmit                         # strict 型別檢查（唯一的 lint/type gate）
npx tsx scripts/selftest.ts              # 純邏輯自測（4 組 ~35 斷言，不需 opencode/mvn/LLM）

# 端到端 pipeline —— 一定要在 Java repo 根目錄（多模組 reactor root）執行：
npx tsx tools/testgen/loop.ts minio-sync-core/src/main/java/<package 或某個 .java>

# 驗證核心零 SDK import（改動 runner 後務必跑）：
grep -rn "@qwen-code/sdk\|@opencode-ai" --include="*.ts" core/ gates/ review/ prompts/ libs/ loop.ts && echo LEAK || echo CLEAN
```

沒有測試框架；`scripts/selftest.ts` 是手寫斷言的純函式自測。改動 `libs/utils.ts`、
`review/verdict.ts`、`gates/coverage.ts` 等純邏輯後先跑它。退出碼：pipeline 成功 0、
失敗 2、致命錯誤 1（`die`）。
...
- **`loop.ts`** — entry point：參數驗證、模組偵測、rubric 載入、startup guard、建立 `runs/<ts>/`。
- **`core/orchestrator.ts`** — 唯一的迭代 loop controller（deterministic，零 SDK import）。
  ...
  4. Review gate：唯讀 reviewer 依注入的 rubric 輸出 JSON 判決（`review/`）

其餘目錄都是被這個 loop 呼叫的無狀態零件：`gates/`（實際跑建置/覆蓋率並解析）、
`review/`（prompt→runner→parse 判決）、`prompts/`（參數化 prompt，注入 standards/rubric）、
`runners/`（AgentRunner 實作）、`libs/`（型別、log、shell、純函式 utils、guard、rubric loader）。
...
3. **Injection over discovery。** standards（writer 契約，`standards/java-ut-standards.md`）
   ...
   對單輪 gate reviewer 是錯誤指令）。搜尋順序：`UT_SKILL_DIR` → `.opencode/skills` → `.claude/skills`。

4. **State in artifacts, not context。** 每 phase 開全新 session，跨輪狀態只落在
   `runs/<ts>/iter-N/`（prompt、writer-summary、build.log、coverage.txt、verdict.json、feedback.md）。
   禁止跨輪複用 session context。
...
| 內容 | 位置 | 消費者 |
| --- | --- | --- |
| 門檻與參數 | `config.ts`（env 覆蓋，自動載入 `.env`） | gates / verdict |
| 測試撰寫標準 | `standards/java-ut-standards.md` | writer prompt |
| 評分 rubric | skill 的 `references/rubric.md` | reviewer prompt |
| 角色契約與權限 | `.opencode/agent/ut-{writer,reviewer}.md`（Java repo 根，不在本目錄） | opencode runtime + startup guard |

門檻與參數**只能改 `config.ts`**（透過 env 覆蓋），不得寫死在 prompt 或 gate 內。

## 修改前必讀的硬規則（違反 = 架構破壞，完整清單見 AGENTS.md §硬規則 / DESIGN.md）

- 迭代控制流只存在於 `core/orchestrator.ts` + `loop.ts`。**禁止**讓 agent 決定重試/停止，
...
- 修改 `libs/types.ts` 共用型別、`review/verdict.ts` 判定邏輯、`runners/` 的 CLI/SDK 呼叫方式、
